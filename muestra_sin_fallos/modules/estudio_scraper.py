...
def format_ah_as_decimal_string_of(ah_line_str: str, for_sheets=False):
    if isinstance(ah_line_str, (int, float)):
        if isinstance(ah_line_str, float) and math.isnan(ah_line_str):
            return '-'
        ah_line_str = f"{ah_line_str}"
    elif not isinstance(ah_line_str, str):
        return '-'

    cleaned_value = ah_line_str.strip()
    if not cleaned_value or cleaned_value in ['-', '?']:
        return cleaned_value if cleaned_value in ['-','?'] else '-'

    numeric_value = parse_ah_to_number_of(cleaned_value)
...
def get_h2h_details_for_original_logic_of(driver, key_match_id, rival_a_id, rival_b_id, rival_a_name="Rival A", rival_b_name="Rival B"):
    ...
    for row in table.find_all("tr", id=re.compile(r"tr2_\d+")):
        ...
        if {h_id, a_id} == {str(rival_a_id), str(rival_b_id)}:
            detalles_match = get_match_details_from_row_of(row, score_class_selector='fscore_', source_table_type='h2h')
            if not detalles_match:
                continue
            score = detalles_match.get('score_raw', '0-0')
            g_h, g_a = score.split("-", 1)
            date_txt = detalles_match.get('date')
            handicap_raw = detalles_match.get('ahLine_raw', 'N/A')
            return {
                "status": "found", "goles_home": g_h.strip(), "goles_away": g_a.strip(),
                "handicap_line_raw": handicap_raw, "match_id": detalles_match.get('matchIndex'),
        ...
        try:
            ...
            if last_home:
                recent_indirect["last_home"] = {
                    ...
                    "ah": format_ah_as_decimal_string_of(last_home.get('handicap_line_raw')),
                    "ou": "-",
            ...
            if last_away:
                recent_indirect["last_away"] = {
                    ...
                    "ah": format_ah_as_decimal_string_of(last_away.get('handicap_line_raw')),
                    "ou": "-",
            ...
            if key_id_a and rival_a_id and rival_b_id:
                ...
                if col3 and col3.get('status') == 'found':
                    ...
                    recent_indirect["h2h_col3"] = {
                        ...
                        "ah": format_ah_as_decimal_string_of(col3.get('handicap_line_raw')),
                        "ou": "-",
    ...
    async with aiohttp.ClientSession(headers=headers) as session:
        try:
            ...
            if last_home:
                ...
                recent_indirect["last_home"] = {"home": last_home.get('home_team'), "away": last_home.get('away_team'), "score": last_home.get('score'), "ah": format_ah_as_decimal_string_of(last_home.get('handicap_line_raw')), "ou": "-", "stats_rows": _df_to_rows(lh_stats_df), "date": last_home.get('date')}
            if last_away:
                la_stats_df = results_map.get('la_stats') if not isinstance(results_map.get('la_stats'), Exception) else None
                recent_indirect["last_away"] = {"home": last_away.get('home_team'), "away": last_away.get('away_team'), "score": last_away.get('score'), "ah": format_ah_as_decimal_string_of(last_away.get('handicap_line_raw')), "ou": "-", "stats_rows": _df_to_rows(la_stats_df), "date": last_away.get('date')}
